#!/usr/bin/env python3
"""Teste de execucao PARALELA das 103 queries TPC-DS num SQL warehouse.

Submete varias queries ao mesmo tempo (thread pool) para medir concorrencia/throughput
do warehouse — diferente do runner serial (03_run_benchmark / run_bench_warehouse.py).
Mede o tempo de cada query E o tempo total do lote (wall clock), e grava em
{catalog}.{schema}.bench_results com engine 'warehouse:<id>:parallel(c=N)'.
...
def one_query(path, args):
    name = os.path.basename(path)[:-4]
    sql = open(path).read().strip().rstrip(";")
    state, secs, rows, err = run_stmt(sql, args, args.catalog, args.schema)
    tag = "ok" if state == "SUCCEEDED" else state
    print(f"  {name:6} {tag:10} {secs:8.2f}s rows={rows} {err}", flush=True)
    return (name, state, secs, rows, err)
...
def main():
    ...
    engine = f"warehouse:{args.warehouse_id}:parallel(c={args.concurrency})"
    print(f"PARALELO: {len(files)} queries, concorrencia={args.concurrency}, warehouse={args.warehouse_id}")
    ...
    with ThreadPoolExecutor(max_workers=args.concurrency) as ex:
        futs = {ex.submit(one_query, p, args): p for p in files}
    ...
    values = ",".join(
        f"('{n}','{st}',{sc},{rw},'{esc(er)}','{run_id}','{engine}',current_timestamp())"
    ...
    sum_secs = sum(r[2] for r in ok)
    print(f"\n=== PARALELO (c={args.concurrency}) ===")
    print(f"{len(ok)}/{len(results)} ok")
    print(f"Wall clock do lote : {wall:.1f}s ({wall/60:.1f} min)")
    print(f"Soma dos tempos    : {sum_secs:.1f}s  (speedup ~{sum_secs/max(wall,0.01):.1f}x)")
    print(f"run_id={run_id} | engine={engine}")
